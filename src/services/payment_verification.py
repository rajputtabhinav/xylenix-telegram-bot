...
class PaymentVerificationService:
    ...
    async def verify_payment_screenshot(self, user_id: int, image_data: bytes, bot_id: str = "paypulse") -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 2: Check for duplicate screenshots ACROSS ALL BOTS
...
Be STRICT. If you're not 100% sure, say "NO"."""
            ...
            logger.info(f"Claude Vision response: {ai_response}")
            
            # STRICT parsing - only approve if Claude explicitly says YES
            if ai_response == "YES":
                return {
                    "verified": True,
                    "message": "✅ **Payment Verified Successfully!**\\n\\n🎉 Welcome to Xylenix! Your account is now verified and you can start earning through referrals.",
                    "details": {"ai_response": ai_response, "verification_method": "claude_vision", "model": settings.anthropic_model}
                }
            else:
                return {
                    "verified": False,
                    "message": "⏳ **Manual Verification Required**\\n\\n📋 Your payment screenshot has been sent to our admin team for verification.\\n\\n⏰ **Processing Time:** Usually within 1-3 hours\\n📱 **You'll be notified** once verification is complete.",
                    "details": {"ai_response": ai_response, "verification_method": "claude_vision", "model": settings.anthropic_model, "reason": "claude_rejected"}
                }
        
        except Exception as e:
            logger.error("Claude Vision verification error: " + str(e))
            # When AI fails, always send to admin for manual verification
            return {
                "verified": False,
                "message": "⏳ **Manual Verification Required**\\n\\n📋 Your payment screenshot has been sent to our admin team for verification.\\n\\n⏰ **Processing Time:** Usually within 1-3 hours\\n📱 **You'll be notified** once verification is complete.",
                "details": {"verification_method": "manual_admin_fallback", "requires_admin_approval": True, "claude_error": str(e)}
            }
